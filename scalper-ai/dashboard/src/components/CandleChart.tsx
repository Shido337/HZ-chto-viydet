...
export const CandleChart: React.FC = () => {
  ...
  // tracks which "symbol:tf" has been fully loaded into the chart
  ...
  // ── 2. Fetch & render full klines when symbol or tf changes ────────────
  useEffect(() => {
    if (!symbol) return;
    const key = `${symbol}:${tf}`;
    loadedKeyRef.current = ''; // mark not loaded while fetching
    ...
    fetch(`/api/klines/${symbol}`)
      ...
      .then((data) => {
        ...
        // Update store so other components (CVD, etc.) stay in sync
        ...
        loadedKeyRef.current = key; // mark loaded
      })
  ...
  // ── 3. Incremental update from WS kline_update events ──────────────────
  useEffect(() => {
    const series = seriesRef.current;
    if (!series || !snap) return;
    // Skip if full data for this symbol:tf hasn't been loaded yet
    if (loadedKeyRef.current !== `${symbol}:${tf}`) return;
  ...
  // ── 4. Price lines (Entry / SL / TP / Trailing / Pending) ──────────────
  useEffect(() => {
    const series = seriesRef.current;
    if (!series) return;
